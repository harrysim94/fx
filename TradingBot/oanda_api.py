...
class OandaAPI:
    ...
    def make_request(
        self, url, params={}, added_headers=None, verb="get", data=None, code_ok=200
    ):
    ...
    def set_sl_tp(self, price, order_type, trade_id):
        ...
        status_code, json_data = self.make_request(
            url, verb="post", data=json.dumps(data), code_ok=201
        )
    ...
    def place_trade(self, pair, units, take_profit=None, stop_loss=None):
        ...
        if (
            "orderFillTransaction" in json_data
            and "tradeOpened" in json_data["orderFillTransaction"]
        ):
