...
def add_chart(pairname, cross, df, writer):
    ...
    chart = get_line_chart(
        workbook, 1, df.shape[0], 8, 9, f"Cum. Gains for {pairname} {cross}", pairname
    )
...
def add_pair_charts(ma_test_res, all_trades, writer):
    ...
    for index, row in df_temp.iterrows():
        temp_all_trades = all_trades[
            (all_trades.CROSS == row.CROSS) & (all_trades.PAIR == row.pair)
        ].copy()
        temp_all_trades["CUM_GAIN"] = temp_all_trades.GAIN.cumsum()
        temp_all_trades[cols].to_excel(
            writer, sheet_name=row.pair, startrow=0, startcol=7
        )
...
def create_excel(ma_test_res, all_trades):
    ...
    ma_test_res = ma_test_res[
        ["pair", "num_trades", "total_gain", "mashort", "malong"]
    ].copy()
    ma_test_res["CROSS"] = (
        "MA_" + ma_test_res.mashort.map(str) + "_" + ma_test_res.malong.map(str)
    )
    ma_test_res.sort_values(
        by=["pair", "total_gain"], ascending=[True, False], inplace=True
    )

    all_trades["CROSS"] = (
        "MA_" + all_trades.MASHORT.map(str) + "_" + all_trades.MALONG.map(str)
    )
